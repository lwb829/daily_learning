...
class AStarPlanner:
    ...
    class Node:
        def __init__(self, x, y, cost, parent_index):
            ...
            self.parent_index = parent_index #父节点在节点列表中的索引
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        """
        # 计算起始节点和目标节点的位置,cost=0,父节点=-1表示没有父节点
        ...
        # openlist和closelist集合,把起始节点'start_node'放入开放集合中
    ...
    # 将给定的位置坐标转换为网格地图中的索引值:计算位置坐标和最小位置坐标之间的差值,在除以网格地图的分辨率,在对结果进行四舍五入,得到整数索引值
    def calc_xy_index(self, position, min_pos):
        return round((position - min_pos) / self.resolution)

    # 计算给定节点在网格地图中的索引:确定节点在y方向上的偏移量,乘以地图宽度;在加上节点在x方向上的偏移量,从而得到索引值
