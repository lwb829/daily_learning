...
分为**时刻（Time）**和**持续时间（Duration）**，且分为**秒（s）**和**纳秒（ns）**，换算关系为：`1nsec=1e-9sec`
...
- 无参构造：
  ...
  - 这个节点句柄的**作用域通常是整个节点**，它是节点的主要句柄
  - 通过这个句柄，你可以创建发布器、订阅器、服务客户端、服务服务器等，并与ROS系统中的其他节点进行通信
  - 这个句柄通常用于**发布全局性的消息或者对整个节点进行配置、管理等操作**
...
- 带有命名空间的构造：
  ...
  - 带有波浪线`("~")`的构造方式表示这个`NodeHandle`对象将使用节点的私有命名空间，它的**作用域是相对于节点的私有命名空间**
  - 通过这个句柄，你可以在节点的私有命名空间中创建发布器、订阅器、服务客户端、服务服务器等，并与ROS系统中的其他节点进行通信
  - 私有命名空间可以让你**在同一个节点内部对不同的组件进行区分**，例如，在一个节点内部创建多个订阅器，它们可以**使用不同的私有参数配置，而不会相互干扰**

  ```
  //使用ROS的参数服务器来获取节点参数的值，举例如下：
  int waypoint_max;
  pnh.param<int>("waypoint_max", waypoint_max, 10000);
  //从私有命名空间中获取名为‘waypoint_max’的参数值，若不存在，则将默认值设为10000，并存储在‘waypoint_max’变量中
  ```
